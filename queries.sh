...
echo "$($PSQL "SELECT wteam.name FROM games INNER JOIN teams AS wteam ON games.winner_id=wteam.team_id WHERE year='2014' AND round='Eighth-Final' UNION SELECT oteam.name FROM games INNER JOIN teams AS oteam ON games.opponent_id=oteam.team_id WHERE year='2014' AND round='Eighth-Final' ORDER BY name")"
